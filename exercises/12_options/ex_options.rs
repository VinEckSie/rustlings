fn main() {
    ...
    match string_slice
        .as_deref()
        .map(|ref_to_string| ref_to_string.as_str())
    {
    ...
    println!(
        "my_vec_processed: {:?}",
        my_vec.iter().flatten().collect::<Vec<_>>()
    );
    ...
    let liquidation_threshold_exp =
        liquidation_threshold.map_or_else(|| map_default(), |lt| lt * 100.0);
    println!(
        "liquidation (default expensive value) {}",
        liquidation_threshold_exp
    );
    ...
    println!(
        "collateral_ratio (got or inserted with): {:?}",
        collateral_ratio
    );
